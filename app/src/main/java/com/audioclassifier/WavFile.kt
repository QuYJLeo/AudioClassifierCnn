...
class WavFile private constructor() {
    ...
    private fun readSample(): Long {
        ...
        for (b in 0 until bytesPerSample) {
            ...
            val v = buffer[bufferPointer].toInt() and 0xFF
    ...
    @Throws(IOException::class, WavFileException::class)
    fun readFrames(sampleBuffer: ShortArray, numFramesToRead: Int): Int {
    ...
    @Throws(IOException::class, WavFileException::class)
    private fun readFramesInternal(sampleBuffer: ShortArray, offset: Int, numFramesToRead: Int): Int {
        var offset = offset
        ...
        for (f in 0 until numFramesToRead) {
            ...
            for (c in 0 until numChannels) {
                ...
                if (offset < sampleBuffer.size) {
                    sampleBuffer[offset] = readSample().toShort()
                    offset++
                } else {
                    // Handle the case where the offset is out of bounds
                    throw WavFileException("ArrayIndexOutOfBoundsException: Offset is out of bounds.")
                }
            }
    ...
    companion object {
        private const val BUFFER_SIZE = 409600
